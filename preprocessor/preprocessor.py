...
class Preprocessor:
    ...
    def process_utterance(self, speaker, basename):
        ...
        """这段代码通过 sum(duration) 将 mel_spectrogram 和 energy 的长度截断为所有语音片段的总长度。
        这样做是为了避免最后一个语音片段的长度可能小于其他片段，导致后面进行 batch 操作时出现维度不一致的情况。"""
        ...
        if self.pitch_phoneme_averaging:
            ...
            """线性插值可以帮助获取更平滑的特征，这在语音合成中非常重要。由于语音信号具有高度复杂的波形，因此在离散的时间步长上采样时
            可能会导致信号的丢失或失真。通过进行插值，可以更好地估计在离散时间步长上未采样的信号值，从而获得更平滑、更连续的语音特征。
            在语音合成任务中，这可以帮助生成更自然、更流畅的语音。"""
    ...
    def average_mel_by_duration(self, mel, duration):
        ...
        x_avg = [
            np.sum(mel[:, int(start):int(end)], axis=1) // np.array(end - start) if len(
                mel[:, int(start):int(end)]) != 0 else mel.zeros()
