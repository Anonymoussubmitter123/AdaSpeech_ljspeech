...
class Preprocessor:
    ...
    def build_from_path(self):
        ...
        """
        标准化是一种常用的数据预处理方法，目的是使得数据在各个特征维度上具有相同的尺度，这有助于提高模型的性能和稳定性。
        StandardScaler是scikit-learn库中的一个标准化类，它可以对数据进行z-score标准化，即将数据按特征列进行标准化，
        使得每列数据的均值为0，标准差为1。
        """
        ...
        for id_language, language in enumerate(tqdm(os.listdir(self.in_dir))):
            for speaker in tqdm(os.listdir(os.path.join(self.in_dir, language))):
                ...
                for wav_name in os.listdir(os.path.join(self.in_dir, language, speaker)):
                    ...
                    basename = wav_name.replace(".wav","")  # 例如，如果wav_name是"audio.wav"，那么basename将是"audio"。
    ...
    def process_utterance(self,language, language_id, speaker, basename):
        ...
        pitch, t = pw.dio(   # 输入为音频，DIO算法主要包括两个步骤：1）首先通过傅里叶变换计算频谱图；2）基于谱峰定位算法进行基频的估计。
            wav.astype(np.float64),
            self.sampling_rate,
            frame_period=self.hop_length / self.sampling_rate * 1000,
        )
        pitch = pw.stonemask(wav.astype(np.float64), pitch, t, self.sampling_rate)  # 平滑基频波形
    ...
    def average_mel_by_duration(self, mel, duration):
        """这个函数的作用是将每个mel spectrogram按照其对应的时长进行平均，返回一个平均后的mel tensor。"""
